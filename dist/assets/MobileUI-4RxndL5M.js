class c{constructor(e){this.engine=e||{qualityManager:{uiAnimationLevel:"low"}},this.uiElements=new Map,this.visibleElements=new Set,this.touchElements=new Map,this.elementPool=new Map,this.batterySaving=!1,this.screenWidth=window.innerWidth,this.screenHeight=window.innerHeight,this.orientationLayout=this.screenWidth>this.screenHeight?"landscape":"portrait",this.devicePixelRatio=window.devicePixelRatio||1,this.hasHapticFeedback="vibrate"in navigator,this.isDeviceSmall=Math.min(this.screenWidth,this.screenHeight)<600,this.frameCounter=0,this.batteryUpdateFrequency=1,this.sizes={buttonSize:this.isDeviceSmall?70:80,joystickSize:this.isDeviceSmall?130:150,healthBarWidth:Math.min(320,this.screenWidth*.6),spacing:this.isDeviceSmall?10:15},this.memoryUsage={elementsCreated:0,activeElements:0,poolSize:0},this.visible=!0,window.addEventListener("resize",this.onResize.bind(this))}initialize(){this.createUIContainer(),this.createSimpleControls(),this.createBatterySavingToggle(),this.ensureMobileMinimapVisibility(),console.log("Mobile UI initialized with simplified user controls")}createUIContainer(){const e=document.createElement("div");e.id="mobile-ui-container",e.style.cssText=`
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        `,i.appendChild(n),e.appendChild(t),e.appendChild(i),this.uiContainer.appendChild(e),this.uiElements.set("batterySaver",e),this.uiElements.set("batterySaverToggle",i),this.uiElements.set("batterySaverHandle",n),this.visibleElements.add("batterySaver"),this.memoryUsage.activeElements+=3,i.addEventListener("touchend",s=>{s.preventDefault(),this.toggleBatterySavingMode(),this.triggerHapticFeedback("toggle")}),this.touchElements.set("battery-toggle",{element:i,type:"toggle",action:"battery"})}ensureMobileMinimapVisibility(){setTimeout(()=>{const e=document.getElementById("minimap-container");if(e)e.style.position="absolute",e.style.top="var(--vc-left-stack-top)",e.style.left="var(--vc-safe-x)",e.style.width="var(--vc-minimap-size)",e.style.height="var(--vc-minimap-size)",e.style.zIndex="1500",e.style.display="block",e.style.opacity="1",e.style.visibility="visible",console.log("Mobile minimap visibility enforced");else{console.warn("Minimap container not found, creating it manually");const t=document.createElement("div");t.id="minimap-container",t.style.cssText=`
                    position: absolute;
